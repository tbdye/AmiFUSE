...
class TestCmdDoctor:
    ...
    def _run_doctor(self, capsys, monkeypatch, checks, args, version="v0.5.0"):
        ...
        monkeypatch.setattr("amifuse.doctor.__version__", version,
                            raising=False)
        # Mock the import of __version__ inside cmd_doctor
        with patch("amifuse.fuse_fs.__version__", version, create=True):
            try:
                cmd_doctor(args)
            except SystemExit:
                pass
    ...
    def test_exit_code_ready(self, monkeypatch):
        ...
        monkeypatch.setattr("amifuse.doctor.run_checks", lambda: checks)
        with patch("amifuse.fuse_fs.__version__", "v0.5.0", create=True):
